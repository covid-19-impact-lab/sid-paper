...
\response{%
    ...
    finalised in June already.

    Many thanks for pointing out that we misclassified the Quidel SARS Sofia antigen
    FIA test; we removed the reference.{\bf \color{red}DOUBLE-CHECK -- this is Smith2021}

    Indeed, in the line of the additional evidence, the numbers we use for test
    sensitivity seem to be in the upper part of plausible values. On
    page~\pageref{r.1.a} of the paper, we cite the two references you pointed out
    (numbering of references relates to this document):
    ...
    in the main text on page~\pageref{r.1.b}

    \textit{\Paste{r.1.b}}

    The robustness is due to the fact that over the course of an infection, the largest differences between the parameters we use in our main analysis and those based on other studies are found during the later stages of an infection. Uncovering an infection that was previously undetected at that stage does
    not have a large effect on subsequent infection dynamics. Our whole analysis in
    Appendix~\ref{subsec:robustness_rapid_test_sensitivity} reads as follows:
...
\begin{enumerate}
    \item They use contact-frequency from pre-pandemic diary data. Is  this truly useful
          ...
          \response{%
              ...
              Many thanks for this comment. Ideally, we would of course use real-time data. For example, several studies have used smartphone tracking data for this purpose. However, they are not detailed enough for our requirements, because they do not provide any information about \textit{who} is meeting with each other. These dimensions make up our contact networks, which are absolutely crucial.
              ...
              at  and for the most part not randomly but in very specific ways. This
              allows us to account for behavior reactions and policy effects that target
              different types of contacts differently. For example, household contacts
              continue to take place no matter the policy environment. Work from home
              does not keep random individuals at home but every individual that works
              has a time-constant value that determines when he or she will stay home.
              Which share of the population stays home is taken from Google mobility
              data. In the education sector we do not use the Mossong data but use
              completely mechanistic models. This gives us confidence that we can
              sufficiently accurately represent the contact patterns during the pandemic
              and account for changes induced by the pandemic.
