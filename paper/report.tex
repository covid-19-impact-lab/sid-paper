...
Since early 2020,\comment[id=J]{\textbf{Features that are not highlighted enough:}

\begin{itemize}
    \item Low sensitivity of rapid tests before and on day of infectiousness onset
    \item Different seasonalities for different contact types (and quite strong for other contacts)
    \item Low number of breakpoints in parameters / policies compared to breakpoints in
    cases / reproduction rate
\end{itemize}

} the CoViD-19 pandemic has presented an enormous challenge to humanity
...
the timing of the test relative to the start of infectiousness.
Figure~\ref{fig:antigen_tests} shows our model for rapid test demand. Schools may
...
because of private reasons becomes higher than compliance at the
workplace.\comment[id=J]{Shouldn't we mention that this is because of availability /
high costs?}
...
Material~\ref{subsec:infection_probs}. We estimate these parameters so that age-specific
simulated infections predict the corresponding time series in the data \citep[see][for
the general method]{McFadden1989}. However, the two sets of time series are not
comparable directly because not all cases will be recorded. We thus model recorded cases
as depicted in Figure~\ref{fig:model_for_official_cases}. We take aggregate estimates of
the share of recorded cases and use data on whether CoViD-19 symptoms led to a PCR test
in order to get age-specific shares. For example,\comment[id=HM]{@Janos, please correct
if necessary and insert reference} estimate unknown cases to be twice as high for
children as for adults. Confirmation of a positive rapid test result is another reason
for PCR tests, once these become available.\comment[id=J]{Free parameters also include:

- One hygiene multiplier for school an work contacts after November 2
- A few "other" multipliers including two lockdown fatigue effects
- Number of extra contacts during holidays

Strictly speaking also:
- The fade in speed of the rapid tests
- Share known cases around holidays }
...
There were many changes in details of regulations afterwards, but they did not change
...
2021.\comment[id=J]{I do not understand} See Supplementary
