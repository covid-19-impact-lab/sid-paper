...
transmitting the virus\comment[id=K]{infection probability per contact for contact type
$c$: $\beta_{c}$}. Contacts occur in up to four networks: Within the household, at work,
...
network-specific frequencies of contacts\comment[id=K]{probability to have $n$ contacts
of contact type $c$: $\eta_{c,\:n}$} from diary data measuring contacts before the
...
Section~\ref{subsec:assortativity}).\comment[id=K]{Share of contacts of age group and
county of i to meet someone of j's age group and county: $\alpha_{work,\:age_i,
age_j,\:county_i,\:county_j}$}
...
contact types. For example, a mandate to work from\comment[id=K]{$\rho_{attend,\,t}$}
...
masking requirements or air filters \citep{Lessler2021}. Curfews\comment[id=K]{would
enter into $\rho_{other\:,t}$} may reduce the number of contacts in settings outside of
...
\citep{Cheng2021}.\comment[id=K]{$\rho_{hygiene}$}
...
\begin{figure}   % Figure 1
    ...
    \floatfoot{\noindent \textit{Note:}
        ...
        influence on an agent's contacts to other agents. Demographic characteristics set
        the baseline number of contacts in different networks ($\eta$). She may reduce
        the number of contacts due to NPIs, showing symptoms, or testing positively for
        SARS-CoV-2 ($\tau$). Infections may occur when a susceptible agent meets an
        infectious agent; the probability depends on the type of contact ($\beta_c$), on
        seasonality ($\kappa_{c}$), and on NPIs ($\rho_{c,\:t}$). If infected, the infection
        ...
        on calendar time ($\pi_{c,\:t}$ and $\tau_{c,\:t}$).
        ...
        uses data on the aggregate share of recorded cases ($\psi$), on the share of
        positive PCR tests triggered by symptoms ($\chi_{symptom}$), and on the false
        positive rate of rapid tests ($p_{positive|infected,\;i,\:t}$). The lower part of
        the graph is relevant only for periods where rapid tests are available.}
    \end{figure}

In the model, susceptibility to contracting the SARS-CoV-2 virus is dependent on
age\comment[id=K]{$\zeta_{a}$}. A possible infection progresses as shown in
Figure~\ref{fig:model_disease_progression}.\comment[id=HM]{New version of figure has
lost the timing. Need to explain d-subscript, maybe even better to add stuff like 1-3
days back in? Needs to be comprehensible without recurring to appendix} We differentiate
...
care, and recovery or death \citep[similar to][]{Grimm2021}.\comment[id=HM]{Need to look
for non-economists -- this must be fairly standard?} The probabilities of transitioning
between these states depend on age; their duration is random and calibrated to medical
literature (for a detailed description see Section~\ref{sub:data_course_of_disease}).
Conditional on the type of contact, infectiousness is independent of age
\citep{Jones2021}.\comment[id=K]{would be $\beta_{c}$}
...
infectiousness\comment[id=K]{$\sigma_v$} can be introduced. Agents may receive a
...
second dose of the Biontech/Pfizer vaccine, which is most commonly used in Germany. We
choose it because our model neither includes booster shots, nor does it allow vaccinated
...
priority may depend on age and occupation; some individuals will refuse vaccination
offers.
...
at least one day to be processed\comment[id=K]{$\gamma_{PCR,\:d}$ where $d=$ number of
days} and there are aggregate capacity constraints. In contrast, rapid antigen tests
yield immediate results; after a phase-in period, all tests that are demanded will be
performed. Specificity\comment[id=K]{$p_{negative|not\:infected}$} and
sensitivity\comment[id=K]{"$p_{positive|infected,\;i,\:t}$", where $i$ is the individual
and $t$ is the date to determine time until/since infectiousness} of these tests is set
...
require staff and students to be tested regularly.\comment[id=K]{"$\pi_{teacher,\:t}$"
etc. } Rapid tests may be offered by employers to on-site workers. Individuals may
demand tests for private reasons, which include having plans to meet other people,
showing symptoms of CoViD-19, and because a household member tested positively for the
virus. We endow each agent with an individual compliance parameter. This parameter
determines whether she takes up rapid tests.\footnote{A positive test result (of either
kind), as well as symptoms leads most individuals to reduce their contacts; this is why
tests impact the actual contacts in Figure~\ref{fig:model_description}.}
...
materials for the precise sources.} Contact diaries yield pre-pandemic distributions of
contacts for different contact types and their assortativity by age group. Mobility data
is used to model the evolution of work contacts. School and daycare policies can be
...
Supplementary Material \ref{subsec:estimated_params}]{McFadden1989}. In our application,
we keep the number of free parameters low in order to avoid overfitting. The data
features to be matched include official case numbers for each age group and region,
...
Figure~\ref{fig:model_official_cases}\comment[id=HM]{Replace P2, P(compliance) by
symbols in figure}. We take aggregate estimates of the share of detected
cases\comment[id=K]{$\psi$} and use data on the share of PCR tests administered because
of CoViD-19 symptoms\comment[id=K]{$\chi_{symptom}$}. As the share of asymptomatic
individuals varies by age group, this gives us age-specific shares (see
...
positive result\comment[id=K]{prob of demanding confirmation is $\chi_{confirmation}$}
is another reason leading to positive PCR tests.
...
\begin{figure}[!tp]   % Figure 2
    ...
    \floatfoot{\noindent \textit{Note:} Data sources are described in Supplementary
        ...
        value of one refers to the situation at the start of our sample period and zero
        ...
        Appendix~\ref{subsec:policies} for separate plots of the three factors by contact
        type.}
\end{figure}
...
\begin{figure}[!tp]
    ...
    \floatfoot{\noindent \textit{Note:} The blue line in
        ...
        remain at their January levels. The other scenarios turn each of these three
        factors on individually. The decompositions in
...
contacts. By the end of our study period, when first-dose vaccination rates reached
around 40\% of the population, the numbers of new cases would have started to decline.
It is important to note that the initial focus of the campaign was to prevent deaths and
severe disease. Indeed, the case fatality was rate considerably lower during the third
wave when compared to the second (4.4\% between October and February and 1.4\% between
March and the end of May).\comment[id=K]{This needs a source.}\comment[id=HM]{Didn't we
calculate that ourselves at some point?}
...
Two of the most contentious NPIs concern schools and mandates to work from
home.\comment[id=K]{We should decide if we want to keep this section or prefer to show
some rapid test statistics.} In many countries, schools switched to remote instruction
...
broadly in line with empirical studies \citep[e.g.,][]{Vlachos2021}. \comment[id=HM]{Need
a back-of the envelope calculation} To use another metric, the effective weekly
