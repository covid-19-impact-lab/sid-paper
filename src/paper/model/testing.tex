...
\label{sub:testing}
 ...
 \comment[id=K]{Talk here about reactions to tests
and symptoms with contact reductions. (If not here, find other place where to talk about
this in detail.)}
...
outside the own household.}\comment[id=K]{I think these multipliers $\tau$ are not
...
Therefore, we \replaced[id=K]{build}{aim for} a simpler model of PCR and rapid tests that
...
tests in the model fits the empirical data well (where empirical data is available).
 \comment[id=K]{Maybe refer to the more detailed \ref{fig:share_rapid_test_last_week2}}
...
strongly depends \replaced[id=K]{on the timing of the rapid test relative to the start of
infectiousness}{on whether the tested individual is already infectious and if so for how
long he has been infectious}. Before the onset of infectiousness the sensitivity is very
...
concern.\comment[id=J]{cite charite study on self administered tests}
...
\citep{Betsch2021}.\comment[id=J]{Need to link to data appendix once the number is there
and maybe mention the number}\comment[id=K]{Not mentioned in the data section.}
