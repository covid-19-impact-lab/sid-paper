...
\begin{enumerate}
    \item They use contact-frequency from pre-pandemic diary data. Is  this truly useful if behavior of people is already changed by the fact there is a pandemic?
          ...
          Other models also use the Mossong data: OpenABM-Covid-19 model (Hinch2020), Bicher2021. 
          
          We only use the pre-pandemic diary data as the starting point. Our policies, which include non-mandated individual behavior adjustments, then reduce contacts taking the type of contact into account and for the most part not randomly but in very specific ways. This allows us to account for behavior reactions and policy effects that target different types of contacts differently. For example, household contacts continue to take place no matter the policy environment. Work from home does not keep random individuals at home but every individual that works has a time-constant value that determines when he or she will stay home. Which share of the population stays home is taken from Google mobility data. In the education sector we do not use the Mossong data but use completely mechanistic models. This gives us confidence that we can sufficiently accurately represent the contact patterns during the pandemic and account for changes induced by the pandemic. 
            
          Also, our excellent fit for most age groups gives us confidence that we manage to match actual meeting patterns. If we were far off for example for the number of work contacts, we would not be able to match either the infections of individuals in the working age (35-59) or of those that mostly do not work anymore (60-79). 
