...
contact networks. The latter probably comes from too optimistic\comment[id=K]{cautious or
slow? I think optimistic might be ambiguous here.} assumptions on school reopenings.
...
\begin{figure}[ht]
  ...
  infectiousness ($\sigma_{B.1.1.7}$). We model no other features of B.1.1.7. We
  introduce at most 0.986 cases per 100,000 inhabitants. The right figure shows the rate
  of individuals that are vaccinated in our synthetic population versus in the general
  German population. Note that we excluded the vaccinations that were given to nursing
  homes (approx. one percent of the German population). Overall, our model covers a time
  frame that goes from zero vaccinated individuals to a state where over 40\% of the
  population are vaccinated. Our vaccinations work imperfectly but we do not model
  different different vaccines nor do we distinguish between first and second shot.}
\end{figure}
