...
The four contact networks are chosen so that the most common NPIs can be modeled in great
...
types. For example, a mandate to work from\comment[id=K]{$\rho_{attend,\,t}$} home will
...
air filters \citep{Lessler2021}. Curfews\comment[id=K]{would enter into $\rho_{other\:,t}$} may reduce the number of contacts in
non-work/non-school/non-work settings. In any setting, measures like masking requirements
would reduce the probability of infection associated with a contact \citep{Cheng2021}.\comment[id=K]{$\rho_{hygiene}$}
...
Figure~\ref{fig:model_disease_progression}. We differentiate between an initial period of
infection without being infectious or showing symptoms, being infectious (presymptomatic
or asymptomatic), showing symptoms, requiring intensive care, and recovery or death
\citep[similar to][]{Grimm2021}.\comment[id=HM]{Need to look for non-economists} The
probabilities of transitioning between these states depend on age;\comment[id=K]{$\gamma$
for all transition probabilities} their duration is random within intervals calibrated to
the medical literature (for a detailed description see Section~\ref{sec:medical_params}).
...
priority may depend on age and occupation.\comment[id=K]{Maybe mention that we have
refusers?}
...
some days to be processed\comment[id=K]{$\gamma_{PCR,\:d}$ where $d=$ number of days} and
...
etc. } Rapid tests may be offered by employers to on-site workers. Individuals may demand
...
for different contact types and their assortativity by age group\comment[id=K]{Should we
add all our sources here?}. Mobility data is used to model the evolution of work
...
\ref{subsec:estimated_params}]{McFadden1989}. In our application, we keep the number of
...
because of CoViD-19 symptoms\comment[id=K]{$\chi_{symptom}$}. As the share of
...
positive result\comment[id=K]{prob of demanding confirmation is $\chi_{confirmation}$} is
...
Material~\ref{subsec:assortativity}).\comment[id=K]{Assortativity is the wrong section
here. They have few contacts in our model because they tend to live in small households
and don't have work or educ contacts. They are also more likely to meet other old people
but that's a different channel from what is written in the text.} By the end of our study
