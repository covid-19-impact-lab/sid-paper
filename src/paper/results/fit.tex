...
this should go to a different section?} and rapid test demand.
...
\begin{figure}[ht]   % observed infections with single runs
  ...
  per million for the reported simulated infections rates. The mean infection rate is the
  ...
  the estimated overall share of detected cases when the second wave hit Germany.
...
group are consistently higher than our simulated case numbers.\comment[id=K]{Does anyone
have an idea why we do not match their shape from Oct to Dec?}
...
\begin{figure}[ht]  % observed infections by age group
  ...
  to 100 year olds live in these facilities. However, the official data does not contain
  information on whether cases were nursing home inhabitants or not. We average over 30
  simulation runs.}
...
be only part of the story why Saxony had such high numbers.}
...
the cost of having less individuals that only occasionally test themselves.
 \comment[id=K]{Write that this makes us underestimate the effect of random tests?}
