...
\begin{enumerate}
    \item They use contact-frequency from pre-pandemic diary data. Is  this truly useful
          ...
          \response{%
              ...
              patterns. If we were far off for, for example, the number of work contacts,
              we would not be able to match either the infections of individuals in the
              working age (35-59) or of those that mostly do not work anymore (60-79).
          }
    \item They state that susceptibility is dependent on age. This requires references
          ...
          \response{%
              Many thanks for pointing this out, indeed we had not cited our key reference for susceptibilities by age group \citep{Davies2020} in the main text. We do so now on Page~\ref{r.2.b}.

              We base our assumption on infectiousness being independent of age on
              \citep{Jones2021}. That paper reports only slight differences in viral
              ...
              differences, if existent, are likely not clinically relevant.

                  {\bf ToDo: Check paper suggestion
                      \url{https://doi.org/10.1093/infdis/jiaa691} -- are they actually checking
                      infectiousness conditional on contact type? Or could we even reproduce
                      what they are doing (not suggesting we should, certainly no need to spend
                      time on it)}
              %   In any case, while the jury is certainly still out on that particular issue, we are confident that different assumptions would not change our results much. 
          }
