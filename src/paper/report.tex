...
\replaced[id=K]{meeting probabilities depend on}{contacts are differentiated by}
age and
...
types. For example, a mandate to work from home will reduce the number of work contacts
to zero for a fraction of the working population. Schools and daycare can be closed
entirely, operate at reduced capacity---including an alternating schedule---, or
...
\begin{figure}   % Figure 1
    ...
    \floatfoot{\noindent \textit{Note:} A        % formal
        ...
        Figure~\ref{fig:model_rapid_tests}. \textcolor{blue}{For each reason only a share
        of eligible individuals receive a test. The share of eligible individuals that
        get tested depends on the reason and changes over time (see
        Figure~\ref{fig:rapid_test_demand}).} \textcolor{red}{All pupils are attending
        school are tested, other reasons trigger a test only for a fraction of
        individuals.} Figure~\ref{fig:model_official_cases} shows the model of
        ...
        share of positive PCR tests triggered by symptoms, \textcolor{red}{and on the
        false positive rate of rapid tests}. The lower part of the graph is relevant only
        for periods where rapid tests are available.}
\end{figure}
...
independent of age \citep{Jones2021}.\comment[id=K]{This might be misleading because the
susceptibility to catch the virus \textbf{is} age-dependent.}
...
that are demanded will be performed.\comment[id=K]{Might be problematic to talk about
supply and demand here. We don't have supply and demand (except for maybe employers and
employees) and I would not interpret the increases in April as an easing of supply side
constraints.} Specificity and sensitivity of these tests is set according to data
analyzed in \cite{Bruemmer2021, Smith2021}; sensitivity depends on the timing of the test
relative to the onset of infectiousness. Figure~\ref{fig:model_rapid_tests} shows our
...
determines whether she takes up rapid tests\deleted[id=K]{offered by employers or
follows up on private reasons} % compliance also determines who is tested in education settings
.\footnote{A positive test result (of either kind), as well as symptoms leads most
...
Figure~\ref{fig:model_official_cases}.\comment[id=K]{I don't understand why we mention
the false positive rates in the floatfoot of this. I would either mention rapid test
sensitivity and specificity in general or not mention them at all.} We take aggregate
...
individuals above age 80 compared to school age children.\comment[id=K]{Must be verified
once results are final.} Once rapid test become available, confirmation of a positive
